...
running_test=$(docker inspect --format="{{.Id}} {{.Config.Image}}" $(docker ps -q) | grep container-discovery-int:)
...
aws_creds=""
compose="docker-compose-int.yaml"
...
while getopts "hipt:a:m:r:" options; do
  case "${options}" in
