...
rails_env_file=""
commit_hash="int"
while getopts "hpr:m:" options; do
  case "${options}" in
    m) image="${OPTARG}" ;;
    p) platform="--platform=linux/amd64"
       image="container-discovery-int-amd64" ;;
...
echo "docker compose -f docker-compose-int.yaml build --build-arg GIT_COMMIT=${commit_hash}"
docker compose -f docker-compose-int.yaml build --build-arg GIT_COMMIT=${commit_hash}
