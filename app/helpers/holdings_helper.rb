module HoldingsHelper
  ...
  def process_online_title(title)
    ...
    title.to_s.gsub(/^Full text available from /, '').gsub(/(\d{1,2})\/\d{1,2}(\/\d{4})/, '\1\2').gsub(/\sConnect to full text\.$/, '').gsub(/(:\s)(\d{1,2}\/\d{4}\sto\s.{,7})/, ' <span class="online-coverage label">\2</span>').html_safe
  end
