...
$(".range_limit .profile .range.slider_js").each(function() {
   ...
   if (isInt(min) && isInt(max)) {
     ...
     var placeholder_input = $('<input type="text" data-slider-placeholder="true" id="slider" style="width:100%;">').appendTo(range_element);
     ...
     if (placeholder_input.slider !== undefined) {
      ...
      if (plot && slider_el) { 
        ...
        slider_el.before('<label class="sr-only" for="slider">Select date range</label>');
      }
      else if (slider_el) {
        slider_el.before('<label class="sr-only" for="slider">Select date range</label>');
