...
#  xml.opensearch :Query, :role => "request", :searchTerms => params[:q], :startPage => page_info.current_page
  ...
  Rails.logger.warn "jgr25_log #{__FILE__} #{__LINE__}: in index.atom.builder”
  <%= warn params %>
  ...
  @document_list.each do |doc|
    xml.entry do
      ...
      xml.link    "rel" => "alternate", "type" => "text/html", "href" => url_for(:controller=>'databases',:action => 'show',  :id => doc.id )
      ...
      xml.id      Rails.root + databases_show_path(doc.id)
