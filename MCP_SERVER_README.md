...
Open `/mcp` in a browser and you get a page instead of the protocol reply: what
the endpoint is, the address to copy, the exact command for each assistant, the
tools with an example prompt for each, and a walkthrough. It is written for a
student pointing an assistant at the catalog, not for a developer.

Its styling lives in the asset pipeline, not in the markup:

| File | What it is |
| ---- | ---------- |
| `app/assets/stylesheets/mcp.scss` | the page's bundle — `cornell/variables`, Bootstrap, Font Awesome, then `mcp/_landing` |
| `app/views/mcp/_brand.html.erb` | the Library lockup at the head of the page |

The page is built from **Bootstrap 5.3 and Font Awesome 4.7**, the same two the
catalog uses, and the bundle imports `cornell/variables` before Bootstrap exactly
as `application.css.scss` does — so `$danger` is Carnelian and a button here
matches a button in the catalog. `mcp/_landing` holds only what Bootstrap has no
utility for. Colour modes are compiled with `$color-mode-type: media-query`, so
the page follows the reader's system theme without a switcher.

It is a standalone bundle (the page renders outside the Blacklight layout), so
it is listed in `config/initializers/assets.rb`. Every rule is scoped to
`body.mcp-landing`, so the bundle cannot restyle the catalog if it is ever
loaded elsewhere. **Adding an MCP asset means adding it to that precompile
list** — production runs with `config.assets.compile = false`, so an unlisted
asset is a 500 on the page, not a missing stylesheet.
...
and `blacklight-cornell/spec/requests/mcp_endpoint_spec.rb`. One README covers
it from the inside:
