...
class SearchBuilder < Blacklight::SearchBuilder
  ...
  def set_query solr_parameters
    ...
    if blacklight_params[:q_row].present?
      ...
      # TODO: Do we need to? Will this conflict with anything if we leave these vars in?
      ...
      # TODO: Test what happens if blacklight_params don't get changed
      # TODO: Test what happens to *_cts search fields
      # TODO: Test what happens to *_browse search fields
      # TODO: Test what happens when filters are added (fq, f)
      # TODO: Test search history and return to search buttons
    end
  end
  ...
  def q_to_solr(query, op, search_field, search_field_config)
    ...
    when 'begins_with'
      ...
      if ['lc_callnum', 'title_starts'].include?(search_field)
        # Don't break up lc_callnum or title_starts queries
  ...
  def q_with_quotes_to_solr(query, op, search_field, search_field_config)
    if ['phrase', 'begins_with'].include?(op) || search_field_config['quoted_field'].nil?
