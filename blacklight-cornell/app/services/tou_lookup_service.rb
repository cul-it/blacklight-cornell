class TouLookupService
  ...
  # Fetches subscription agreements linked to a specific package.
  ...
  # Fetches a single license by remoteId from FOLIO licenses app.
  def license(remote_id)
    folio_request("#{ENV['OKAPI_URL']}/licenses/licenses/#{remote_id}")
  end

  # Performs a GET to a FOLIO endpoint with Okapi headers.
  ...
  # Resolve TOU using the "database" core by ID. Mirrors DatabasesController logic.
  ...
  def resolve_for_database(id:)
    ...
    if blank?(dbcode)
      ...
      return build_result(db_docs, [], 'Use default rights text') if blank?(providercode)
  ...
  def resolve_catalog_terms_of_use(id:, dbcode:, providercode:)
    ...
    dblinks.each do |link_str|
      ...
      if normalize(link_provider) == normalize(providercode) && normalize(link_dbcode) == normalize(dbcode)
        default = ''
  ...
  # Small helper to normalize strings for equality checks.
  def normalize(value)
    value.to_s.strip
  end

  # Predicate for nil/empty strings.
  def blank?(value)
    value.nil? || value == ''
  end
