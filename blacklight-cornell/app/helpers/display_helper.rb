module DisplayHelper
  ...
  def render_clickable_item args, value
    ...
    when String
      link_to(value, add_search_params(args[:field], '"' + value + '"')) # default single value
    when Hash
      # delimited value to be separated out further
      if clickable_setting[:sep] != nil
        value_array = value.split(clickable_setting[:sep]) # separator defined
        ...
        if clickable_setting[:key_value]
          displayv_searchv = value.split(clickable_setting[:sep]) # field has display value and search value separated by :sep
          if displayv_searchv.size > 2
            ...
            if !clickable_setting[:related_search_field].blank?
              ...
              related_auth_val = args[:document][clickable_setting[:related_auth_field]] # include optional link to authority browse info
              ...
              # misconfiguration... no related search field defined # ignore related search value
            ...
            content_tag('span', displayv_searchv[0]) # display only
          end
        elsif clickable_setting[:hierarchical]
          hierarchical_value = '' # fields such as subject are hierarchical
  ...
  def get_clickable_search_field field
    ...
    case clickable_setting
    when String
      return clickable_setting # default single value
    when Hash
      return clickable_setting[:search_field] # delimited value to be separated out further
    end
  end
  ...
  def render_show_format_value field
    ...
    field[:value].map do |f|
      icon = '<i class="fa fa-' + formats_icon_mapping(f) + '"></i> ' # Convert format to array in case it's a string (it shouldn't be)
  ...
  def render_format_value args
    ...
    format = [format] unless format.is_a? Array # Convert format to array in case it's a string (it shouldn't be)
    format.map do |f|
  ...
  def link_to_document(doc, field_or_opts = nil, opts = { :label => nil, :counter => nil, :results_view => true })
    if (['bookmarks', 'book_bags'].include?(params[:controller])) && !folio_record?(doc)
      label = field_or_opts
      docID = doc.id
  ...
  def is_exportable document
    return false if folio_record?(document)
  ...
  def render_document_index_label doc, field, opts = {}
    if field.kind_of? Hash
      field = field[:label] #  Deprecation.warn self, "Calling render_document_index_label with a hash is deprecated"
    end
  ...
  def render_facet_value(facet_solr_field, item, options = {})
    ...
    if facet_solr_field != 'format'
      ...
      content_tag(:span, :class => "facet-label") do
        (facet_icon).html_safe + link_to_unless(options[:suppress_link], facet_display_value(facet_solr_field, item), path, :class => "facet_select")
      end + render_facet_count(item.hits)
    end
  end

  def simple_render_index_field_value * args
    simple_render_field_value(*args) # simple_ are versions of deprecated functions
  end
  ...
  def render_head_content
    Deprecation.silence(Blacklight::HtmlHeadHelperBehavior) do
      render_stylesheet_includes + render_js_includes + render_extra_head_content
    end +
      content_for(:head)
  end
  ...
  def alert_messages(path)
    begin
      ...
      alert_messages.each do |m|
        ...
        regex = m['pages'].present? && m['pages'] != [] ? Regexp.union(m['pages']) : Regexp.new('.*')
