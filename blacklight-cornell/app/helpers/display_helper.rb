...
# @param [Symbol, Proc, String] field Render the given field or evaluate the proc or render the given string
 ...
 def cornell_params_for_search(*args, &block)
      ...
      when 2
        BlacklightCornell::SearchState.new(args.first, blacklight_config).params_for_search(args.last)
      else
    ...
    def cornell_reset_search_params(source_params)
      BlacklightCornell::SearchState.new(source_params, blacklight_config).send(:reset_search_params)
    end

    def cornell_add_facet_params(field, item, source_params = nil)
      if source_params

        BlacklightCornell::SearchState.new(source_params, blacklight_config).add_facet_params(field, item)
      else
    ...
    def cornell_remove_facet_params(field, item, source_params = nil)
      if source_params
        BlacklightCornell::SearchState.new(source_params, blacklight_config).remove_facet_params(field, item)
      else
