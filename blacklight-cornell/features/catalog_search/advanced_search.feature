...
Feature: Search
 ...
 @javascript
  Scenario: Perform a 2 row  advanced search with Title, with percent that must be url encoded.
    ...
    And I press 'advanced_search'
    Then I should get results
    Then it should have link "Title: beef" with value '/catalog?action=index&advanced_query=yes&commit=Search&controller=catalog&op_row%5B%5D=AND&q=title+%3D+100%25&q_row%5B%5D=100%25&range%5Bpub_date_facet%5D%5Bbegin%5D=&range%5Bpub_date_facet%5D%5Bend%5D=&search_field=advanced&search_field_row%5B%5D=title&sort=score+desc%2C+pub_date_sort+desc%2C+title_sort+asc&y='
