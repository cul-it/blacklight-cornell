...
module BlacklightCornell::Discogs extend Blacklight::Catalog
  ...
  def process_discogs(doc)
    ...
    if fields["title"].present? && (fields["author"].present? || fields["title_responsibility"].present?)
      ...
      if search_result.present? && search_result["results"].present?
  ...
  def build_discogs_query_string(fields)
    ...
    if author.length > 0
      ...
      if fields["single_author"]
        ...
        if !query_string.include?(first_last)
          query_string += "+" + first_last
        end
      elsif !query_string.include?(author)
        query_string += "+" + author
      end
    else
      query_string += "+" + fields["title_responsibility"]
    end
