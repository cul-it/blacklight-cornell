...
#############################
NUM_PROCESSES=${NUM_PROCESSES:-1} # Default to 1 if NUM_PROCESSES is not set
...
if [ -z ${CUCUMBER_FEATURE_TESTS+x} ]; then
    ...
    bundle exec parallel_cucumber features/ -n $NUM_PROCESSES --test-options "$OPT1 $OPT2 $OPT3 $OPT4"
else
    echo "Running feature: ${CUCUMBER_FEATURE_TESTS}"
    bundle exec parallel_cucumber "${CUCUMBER_FEATURE_TESTS}" -n $NUM_PROCESSES --test-options "$OPT1 $OPT2 $OPT3 $OPT4"
fi