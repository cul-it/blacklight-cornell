...
Given /^I request the personal name author item view for (.*?)$/ do |author|
	do_visit "/browse/info?authq=#{author}&browse_type=Author&headingtype=Personal%20Name"
end

Given /^I request the geographic name subject item view for (.*?)$/ do |subject|
	do_visit "/browse/info?authq=#{subject}&browse_type=Subject&headingtype=Geographic%20Name"
end

Given /^I request the author title item view for (.*?)$/ do |at|
	do_visit "/browse/info?authq=#{at}&browse_type=Author-Title"
end
