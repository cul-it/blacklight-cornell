...
Given /^I request the personal name author item view as (.*?)$/ do |author|
  visit "/browse/info?authq=#{author}&browse_type=Author&headingtype=Personal%20Name"
end

Given /^I request the geographic name subject item view as (.*?)$/ do |subject|
  visit "/browse/info?authq=#{subject}&browse_type=Subject&headingtype=Geographic%20Name"
end

Given /^I request the author title item view as (.*?)$/ do |at|
