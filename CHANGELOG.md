...
- Add automated changelog generation GitHub Actions workflow with draft and official release modes by @JeremyDuncan in [#2458](https://github.com/cul-it/blacklight-cornell/pull/2458) ([DACCESS-907](https://culibrary.atlassian.net/browse/DACCESS-907)) (#2458)
- Add structured PR template with type classification, Jira linking, and reviewer testing instructions by @JeremyDuncan in [#2455](https://github.com/cul-it/blacklight-cornell/pull/2455) (#2455)

### 🐛 Bug Fixes

- Fix 'View all Catalog results' link in bento search to correctly include the search_field parameter and improve special-character URL encoding by @ebtoner in [#2476](https://github.com/cul-it/blacklight-cornell/pull/2476) ([DACCESS-916](https://culibrary.atlassian.net/browse/DACCESS-916)) (#2476)
- Fix changelog workflow concurrency issue that caused required workflows to cancel each other, and improve changelog PR tracking for active dev-to-main releases by @JeremyDuncan in [#2473](https://github.com/cul-it/blacklight-cornell/pull/2473) ([DACCESS-907](https://culibrary.atlassian.net/browse/DACCESS-907)) (#2473)
- Fix call number facet async loading when search queries contain forward slashes, backslashes, or double-encoded spaces by @ebtoner in [#2472](https://github.com/cul-it/blacklight-cornell/pull/2472) ([DACCESS-882](https://culibrary.atlassian.net/browse/DACCESS-882)) (#2472)
...
- Fix Warden errors in controller specs by including Devise test helpers so initialize_bookbag_state runs correctly in test environments by @JeremyDuncan in [#2465](https://github.com/cul-it/blacklight-cornell/pull/2465) ([DACCESS-840](https://culibrary.atlassian.net/browse/DACCESS-840)) (#2465)
- Fix browser back button breaking in bento search by removing legacy anchor scroll JavaScript by @mhk33 in [#2463](https://github.com/cul-it/blacklight-cornell/pull/2463) ([DACCESS-868](https://culibrary.atlassian.net/browse/DACCESS-868)) (#2463)
- Fix Book Bag item count not displaying on the homepage until user visits the bookbag view by @JeremyDuncan in [#2462](https://github.com/cul-it/blacklight-cornell/pull/2462) ([DACCESS-840](https://culibrary.atlassian.net/browse/DACCESS-840)) (#2462)

### 🦮 Accessibility

- Fix selected items list role attribute to update dynamically when items are removed or the form is reset on special request forms by @sarah-cul in [#2475](https://github.com/cul-it/blacklight-cornell/pull/2475) ([DACCESS-895](https://culibrary.atlassian.net/browse/DACCESS-895)) (#2475)
- Fix multiple accessibility issues on item record and special request form pages including empty container elements, missing ARIA landmarks, header order, and keyboard accessibility for disabled checkboxes by @sarah-cul in [#2467](https://github.com/cul-it/blacklight-cornell/pull/2467) ([DACCESS-896](https://culibrary.atlassian.net/browse/DACCESS-896), [DACCESS-895](https://culibrary.atlassian.net/browse/DACCESS-895), [DACCESS-908](https://culibrary.atlassian.net/browse/DACCESS-908), [DACCESS-898](https://culibrary.atlassian.net/browse/DACCESS-898), [DACCESS-915](https://culibrary.atlassian.net/browse/DACCESS-915)) (#2467)
- Change fixed font sizes to relative units in Databases, Digital Collections, Terms of Use, and Footer sections to improve SiteImprove accessibility score by @mhk33 in [#2459](https://github.com/cul-it/blacklight-cornell/pull/2459) ([DACCESS-894](https://culibrary.atlassian.net/browse/DACCESS-894)) (#2459)
- Fix aria-hidden typo in availability box, italics rendering in databases section, and improper heading usage for subtitle and author on item view by @mhk33 in [#2456](https://github.com/cul-it/blacklight-cornell/pull/2456) ([DACCESS-899](https://culibrary.atlassian.net/browse/DACCESS-899)) (#2456)

### 🧰 Maintenance

- Add automated changelog PR as part of the dev-to-main release process by @github-actions[bot] in [#2481](https://github.com/cul-it/blacklight-cornell/pull/2481) (#2481)
- Bump devise from 5.0.3 to 5.0.4 to fix open redirect security vulnerability in FailureApp via unvalidated Referer header (CVE-2026-40295) by @dependabot[bot] in [#2478](https://github.com/cul-it/blacklight-cornell/pull/2478) (#2478)
- Update Chromium version and unpin dependencies to prevent test build failures, and add early error detection and manual --no-cache rebuild support in CI by @JeremyDuncan in [#2477](https://github.com/cul-it/blacklight-cornell/pull/2477) (#2477)
- Improve changelog automation workflow with consistent section ordering, newest-to-oldest PR ordering, PR-based changelog updates, and clearer release-note guidance by @JeremyDuncan in [#2468](https://github.com/cul-it/blacklight-cornell/pull/2468) ([DACCESS-907](https://culibrary.atlassian.net/browse/DACCESS-907)) (#2468)
- Replace deprecated document_show_fields rendering with Blacklight's DocumentMetadataComponent to resolve Blacklight 8 deprecation warnings by @chrisrlc in [#2461](https://github.com/cul-it/blacklight-cornell/pull/2461) ([DACCESS-824](https://culibrary.atlassian.net/browse/DACCESS-824), [DACCESS-825](https://culibrary.atlassian.net/browse/DACCESS-825)) (#2461)
- Fix flaky JavaScript-loaded Cucumber test by ensuring page paths are always root-relative to prevent state-dependent navigation failures by @JeremyDuncan in [#2457](https://github.com/cul-it/blacklight-cornell/pull/2457) ([DACCESS-644](https://culibrary.atlassian.net/browse/DACCESS-644)) (#2457)
- Upgrade application to Rails 8, update related gems for compatibility, and add custom error pages for 422 and 400 responses by @chrisrlc in [#2453](https://github.com/cul-it/blacklight-cornell/pull/2453) ([DACCESS-856](https://culibrary.atlassian.net/browse/DACCESS-856), [DACCESS-887](https://culibrary.atlassian.net/browse/DACCESS-887)) (#2453)
- Fix Jenkins CI pipeline so test coverage files are written with correct ownership, enabling clean workspace between builds by @JeremyDuncan in [#2438](https://github.com/cul-it/blacklight-cornell/pull/2438) ([DACCESS-737](https://culibrary.atlassian.net/browse/DACCESS-737)) (#2438)
- Remove deprecated filter_params and query_has_constraints? overrides and replace with a safelist of non-public facets in the catalog controller by @ebtoner in [#2401](https://github.com/cul-it/blacklight-cornell/pull/2401) ([DACCESS-750](https://culibrary.atlassian.net/browse/DACCESS-750), [DACCESS-755](https://culibrary.atlassian.net/browse/DACCESS-755)) (#2401)
