...
describe RequestController, :type => :controller do
	...
	describe "make_request" do
		...
		render_views
			it "renders the flash_msg partial" do
				param = { :request_action => 'callslip' }
				get :make_request
				response.should render_template(:partial => '/flash_msg', :layout => false)
			end	
	...
	describe "_get_item_type" do
		...
		let(:day_loan_types) 	{ RequestController::IRREGULAR_LOAN_TYPE[:DAY].keys 	  }
		let(:minute_loan_types) { RequestController::IRREGULAR_LOAN_TYPE[:MINUTE].keys }
		let(:nocirc_loan_types) { RequestController::IRREGULAR_LOAN_TYPE[:NOCIRC].keys }
		let(:rc)                { RequestController.new }

		it "returns 'day' for a day-type loan" do
			day_loan_types.each do |t|
				response = rc._get_item_type 'typeCode' => t
				response.should == 'day'
			end
		end
